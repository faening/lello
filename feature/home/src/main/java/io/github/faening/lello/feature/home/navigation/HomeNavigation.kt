...
fun NavGraphBuilder.homeGraph(navController: NavController) {
    ...
    composable(route = HomeRoutes.HOME_MAIN_ROUTE) {
        HomeScreen(
            onFeatureClick = { featureId ->
                // Navegar para um recurso específico dentro do Home
                navController.navigate("${HomeRoutes.HOME_FEATURE_ROUTE}/$featureId")
            },
            onDetailClick = { itemId ->
                // Navegar para detalhes de um item
                navController.navigate("${HomeRoutes.HOME_DETAIL_ROUTE}/$itemId")
            },
    ...
    composable(
        route = "${HomeRoutes.HOME_DETAIL_ROUTE}/{itemId}"
    ) { backStackEntry ->
    ...
    composable(
        route = "${HomeRoutes.HOME_FEATURE_ROUTE}/{featureId}"
    ) { backStackEntry ->
