...
class SleepDurationOptionUseCase @Inject constructor(
    private val repository: OptionResources<SleepDurationOption>
) {
    ...
    ): Flow<List<SleepDurationOption>> {
        return repository.getAll(useBlockedFilter, isBlocked, useActiveFilter, isActive)
    }

    fun getById(id: Long): Flow<SleepDurationOption>? {
        id.validateId()
        return repository.getById(id)
    }

    suspend fun save(vararg items: SleepDurationOption) {
    ...
    suspend fun update(vararg items: SleepDurationOption) {
        val formattedItems = items.map { item ->
     ...
     */
    suspend fun updateActiveStatus(vararg items: SleepDurationOption) {
        items.forEach { item ->
            item.id.validateId()
        }
    ...
    suspend fun delete(vararg items: SleepDurationOption) {
        items.forEach { item ->
            item.blocked.validateNotBlocked()
            item.id.validateId()
        }
