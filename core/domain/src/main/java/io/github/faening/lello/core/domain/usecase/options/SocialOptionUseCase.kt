...
class SocialOptionUseCase @Inject constructor(
    private val repository: OptionResources<SocialOption>
) {
    ...
    fun getById(id: Long): Flow<SocialOption>? {
        id.validateId()
        return repository.getById(id)
    }

    suspend fun save(vararg items: SocialOption) {
    ...
    suspend fun update(vararg items: SocialOption) {
        val formattedItems = items.map { item ->
